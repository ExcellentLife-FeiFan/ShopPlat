...
import com.ytxd.spp.util.Convert;
    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != ApiResult.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                ApiResult2 simpleResponse = Convert.fromJson(jsonReader, ApiResult2.class);
                response.close();
                //noinspection unchecked
                return (T) simpleResponse;
            } else {
                // 泛型格式如下： new JsonCallback<LzyResponse<内层JavaBean>>(this)
                ApiResult lzyResponse = Convert.fromJson(jsonReader, type);
                response.close();
                int code = lzyResponse.getState();
