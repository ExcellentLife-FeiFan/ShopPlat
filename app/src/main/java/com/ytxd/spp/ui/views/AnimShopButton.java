...
public class AnimShopButton extends View {
    ...
    protected Region mAddRegion2, mDelRegion2;
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mAddRegion2 = new Region();
        mDelRegion2 = new Region();
        mAddPath = new Path();
        mDelPath = new Path();
...
/*        mMaxCount = 4;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mWidth = w;
        mHeight = h;
        mDelRegion2.set(0, 0, mWidth / 2, mHeight);
        mAddRegion2.set(mWidth / 2, 0, mWidth, mHeight);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!ignoreHintArea && isHintMode) {
            ...
            RectF rectF = new RectF(mLeft + (mWidth - mRadius * 2) * mAnimExpandHintFraction, mTop
                    , mWidth - mCircleWidth-getPaddingRight(), mHeight - mCircleWidth-getPaddingBottom());
        ...
        /*canvas.drawLine(mAnimOffset + mLeft + mRadius / 2, mTop + mRadius,
            ...
            Path mAddPath2 = new Path();
            mAddPath2.addCircle(left + mRadius, mTop + mRadius, mRadius + 50, Path.Direction.CW);
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (isHintMode) {
                    ...
                    if (mAddRegion.contains((int) event.getX(), (int) event.getY())) {
                        ...
                        if (mAddRegion2.contains((int) event.getX(), (int) event.getY())) {
                            onAddClick();
                            return true;
                        } else if (mDelRegion2.contains((int) event.getX(), (int) event.getY())) {
                            onDelClick();
                            return true;
                        }
                    }
